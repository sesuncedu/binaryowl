...
public class BinaryOWLMetadata {
    ...
    private void read(BinaryOWLInputStream inputStream) throws IOException, BinaryOWLParseException {
        byte mapCount = inputStream.readByte();
        if(mapCount == 0) {
            return;
        }
    ...
    private abstract class AttributeMap<O> implements Map<String, O>{
        ...
        protected abstract O readValue(BinaryOWLInputStream inputStream) throws IOException, BinaryOWLParseException;
        
        protected abstract void writeValue(O value, BinaryOWLOutputStream outputStream) throws IOException;
